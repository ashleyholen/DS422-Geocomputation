...
options(tigris_use_cache = TRUE) # speeds up repeated runs

# ensure fips is 5 characters (with leading zeros if needed)
chr_data <- chr_data %>%
  mutate(fips = str_pad(fips, width = 5, side = "left"))
...
ggplot(map_data) +
  ...
  labs(
    title = "Percent Fair or Poor Health by County",
    fill = "% Fair or Poor Health",
    caption = "Source: County Health Rankings data"
  ) +
  theme_minimal() +
  theme(
